...
//-------------------------------------------------- CutOut ----------------------------------------------------
void setDCCBitCutOut(rmt_item32_t* item) {                     // * CutOut time fixed at 488µs (TCE Max.)
  item->level0    = 1;
...
void IRAM_ATTR interrupt(rmt_channel_t channel, void *t) {
  if (channel == 0) {                                            // * Main
    gpio_matrix_out(GPIO_NUM_12, 0x100, false, false);           // * Disconnect pin 12 to module RMT
    gpio_set_level(GPIO_NUM_12, 1);                              // * Pin 12 high
    canalMain = channel;                                         // * Save n° canal
    StarTimerCutOutMain();                                       // * Start Timer CutOut Main
  }
  else {                                                         // * Prog
    gpio_matrix_out(GPIO_NUM_33, 0x100, false, false);           // * Disconnect pin 33 to module RMT
    gpio_set_level(GPIO_NUM_33, 1);                              // * Pin 33 high
    canalProg = channel;                                         // * Save n° de canal
...
RMTChannel::RMTChannel(pinpair pins, bool isMain) {
  ...
  setDCCBitCutOut(preamble);                                     // * Symbol CutOut     
  for (byte n = 1; n < plen; n++)                                // * !!! n = 1
