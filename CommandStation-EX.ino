...
hw_timer_t * TimerCutOutProg = NULL;                                          // * Timer
...
void IRAM_ATTR timer_isr_CutOutMain() {
  ...
  switch (p) {
    case 1:
      break;
    case 2:
      gpio_matrix_out(GPIO_NUM_14, 0x100, false, false);                         // * Disconnect pin 14 to module RMT
      gpio_set_level(GPIO_NUM_14, 1);                                            // * Pin 14 high
      break;
    case 3:
      gpio_matrix_out(GPIO_NUM_14, RMT_SIG_OUT0_IDX + canalMain, true, false);    // * Reconnect pin 14 to module RMT in reverse
      gpio_matrix_out(GPIO_NUM_12, RMT_SIG_OUT0_IDX + canalMain, true, false);    // * Reconnect pin 12 to module RMT in reverse
      break;
    default:
      gpio_set_level(GPIO_NUM_12, 1);                                             // * Pin 14 high
      gpio_matrix_out(GPIO_NUM_12, RMT_SIG_OUT0_IDX + canalMain, false, false);   // * Reconnect pin 12 to module RMT
      timerAlarmWrite(TimerCutOutMain, 160, false);                               // * Stop Timer
      break;
  }
}
void IRAM_ATTR timer_isr_CutOutProg() {
  ...
  switch (q) {
    case 1:
      break;
    case 2:
      gpio_matrix_out(GPIO_NUM_27, 0x100, false, false);                          // * Disconnect pin 27 to module RMT
      gpio_set_level(GPIO_NUM_27, 1);                                             // * Pin 27 high
      break;
    case 3:
      gpio_matrix_out(GPIO_NUM_27, RMT_SIG_OUT0_IDX + canalProg, true, false);    // * Reconnect pin 27 to module RMT in reverse
      gpio_matrix_out(GPIO_NUM_33, RMT_SIG_OUT0_IDX + canalProg, true, false);    // * Reconnect pin 33 to module RMT in reverse
      break;
    default:
      gpio_set_level(GPIO_NUM_33, 1);                                             // * Pin 27 high
      gpio_matrix_out(GPIO_NUM_33, RMT_SIG_OUT0_IDX + canalProg, false, false);   // * Reconnect pin 33 to module RMT
      timerAlarmWrite(TimerCutOutProg, 160, false);                               // * Stop Timer
      break;
  }
}
//----------------------------------------------------------------------
...
void StarTimerCutOutMain() {
  timerAlarmWrite(TimerCutOutMain, 160, true);
  timerAlarmEnable(TimerCutOutMain);
  p = 0;
}
void StarTimerCutOutProg() {
  timerAlarmWrite(TimerCutOutProg, 160, true);
  timerAlarmEnable(TimerCutOutProg);
  q = 0;
}
//------------------------------------------------------------------------------------------------
